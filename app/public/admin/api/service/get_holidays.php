...
if (session_status() == PHP_SESSION_ACTIVE && $_SESSION['logged'] && $_SESSION['isAdmin']) {
    ...
    try {
        if (isset($_GET['serviceId']) && is_numeric($_GET['serviceId']) && !empty($_GET['serviceId']) && isset($_GET['date'])) {
            $holidays = \Admin\Services::searchHolidays($db, $_GET['serviceId'], str_replace(["/", "-", " "], "%", $_GET['date']));
